...
double r1_distanceToClusters(const double *elem, double **centers, const bool *asso,
                             unsigned nb_clusters, unsigned nb_dim, double alpha) {
...
void r1_assign(const double *const *elements, double **centers, bool **asso, unsigned nb_elements,
               unsigned nb_clusters, unsigned nb_dim, double alpha, double *withinss) {
    ...
    for (unsigned i = 0; i < nb_elements; i++) {
        ...
        do {
            ...
            double next_new_dist =
                r1_distanceToClusters(elements[i], centers, tmp_asso, nb_clusters, nb_dim, alpha);
...
void r1_update(const double *const *elements, double **centers, bool **asso, unsigned nb_elements,
               unsigned nb_clusters, unsigned nb_dim, double alpha, double *withinss) {
    ...
    for (unsigned i = 0; i < nb_elements; i++) {
        withinss[i] =
            r1_distanceToClusters(elements[i], centers, asso[i], nb_clusters, nb_dim, alpha);
    }
}

// == R1-OKM ==

void r1okm(const double *const *elements, double **centers, bool **asso, const unsigned nb_elements,
           const unsigned nb_clusters, const unsigned nb_dim, const double alpha, const bool trace,
    ...
    *totwss = INFINITY;
    ...
    do {
        ...
        r1_assign(elements, centers, asso, nb_elements, nb_clusters, nb_dim, alpha, withinss);
        const double va = sum_double_array(withinss, nb_elements);

        // Update all centers
        r1_update(elements, centers, asso, nb_elements, nb_clusters, nb_dim, alpha, withinss);
